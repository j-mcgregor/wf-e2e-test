...
const SmeCalc = () => {
  ...
  const headings = useReportNavItems()
  return (
    <Layout title="SME - Calculator " fullWidth>
      <SecondaryLayout navigation={ <ReportNav companyName={'Scottish Seabird Company LTD'} />} >
       {headings.map(header => <div data-report-section="true" id={`${header.toLowerCase().replace(/ /g, '-')}-id`} key={header} className="h-screen text-3xl pt-16"><HashHeader text={header} /></div>) }
      </SecondaryLayout>
    </Layout>
  )
}
