...
const buttonClassName = {
  ...
  highlight: "bg-highlight hover:bg-yellow-500 focus:ring-yellow-500 text-white ", // orange
}
...
const Button = ({
  ...
  if (linkTo) {
    return <Link linkTo={linkTo} className={newClassName ? newClassName : `${buttonClassName.base} ${buttonClassName[variant]} ${className}`}
    > 
      {children}
    </Link>
  }
  ...
  return (
    <button
      onClick={onClick}
      type={type}
      className={newClassName ? newClassName : `${buttonClassName.base} ${buttonClassName[variant]} ${className}`}
    >
      {loading && <LoadingIcon />}
      {!loading && children}
    </button>
