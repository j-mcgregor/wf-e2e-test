...
const SummaryMap = ( props: SummaryContact) => {
  const t = useTranslations();

  const phone = props?.phone_numbers[0].replace(/[A-z-]/g, '');
      ...
      <div className="flex flex-col p-4">
        <div>
          ...
          <div className="font-bold pb-2">
            ...
            <p>{props?.address_line_2}</p>
            <p>{props?.address_line_3}</p>
            <p>{props?.address_line_4}</p>
        ...
        <div className="flex justify-between">
          ...
          {/* {props?.emails && <p>{props.emails[0]}</p>} */}
          <p className="font-bold">info@writeglazing.com</p>
        </div>
        <div className="flex justify-between">
          <p>{t('email')}</p>
          {props?.websites && (
            <p className="font-bold">{props.websites[0]}</p>
          )}
        </div>
        <div className="flex justify-between">
          <p>{t('telephone')}</p>
          {props?.phone_numbers && <p className="font-bold">{phone}</p>}
        </div>
      </div>
    </div>
  );
